...
let
  ...
  warnMissing = path: meta:
    let
      ...
      # Strict warning criteria:
      # 1. Starts with pkgs.zenos
      # 2. Starts with zenos AND does NOT contain legacy
  ...
  showPackages =
    ...
    let
      ...
      rawBrief =
        if isZenPkg then v.brief
        else if isDrv then (v.meta.description or null)
        else if isLegacy then "legacy package/set"
        else null;

      rawDescription =
        if isZenPkg then v.description or null
        else if isDrv then (v.meta.longDescription or v.meta.description or null)
        else null;
    ...
    if isRepeating || depth > maxDepth then
      {
        meta = metaObj // { debug = "recursion blocked"; };
        sub = { };
      }
    else
      {
  ...
  showOptions =
    ...
    in
    if isRepeating then
      { meta = { type = { name = "unknown"; }; brief = "Infinite loop detected"; }; }
    else
      let
        ...
        getRawChildren = v:
          ...
          else if (builtins.length path == 4) && (name == "legacy") && (builtins.elemAt path 1 == "users") then eval.options.users.users.type.nestedTypes.elemType.getSubOptions [ ]
        ...
        validChildren = if rawChildren != null then builtins.removeAttrs rawChildren ([ "_module" "_args" "freeformType" "specialisation" "containers" "vmVariant" ] ++ (if path == [ "zenos" ] then [ "sandbox" ] else [ ])) else { };
      in
      {
        meta = metaObj;
      }
