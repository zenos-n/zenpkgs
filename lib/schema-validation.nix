...
let
  ...
  option = path: type: annotation: {
    ...
    inherit annotation;
    typeName = type.name;
    checks = checkLiterals type (builtins.filter (request: request.path == path) requests);
  };
  ...
  packages = depth: path: tree:
    ...
    let result = attempt (
      if !builtins.isAttrs tree then unsupported "not a package or package set"
        ...
        # Selector type is always bool, independent of the freeform user's name.
        # General freeform option types may depend on name and stay deferred.
        checks = checkLiterals lib.types.bool (builtins.filter (request:
          builtins.length path == builtins.length request.path
          && builtins.all (x: x) (lib.zipListsWith (pattern: key: pattern == "<name>" || pattern == key) path request.path)
        ) requests);
      }
  ...
  typed = depth: path: type:
    ...
    if name == "zstr-package-selectors" then
      ...
      selected // lib.optionalAttrs (selected.kind == "branch" && packageTree ? legacy) {
        children = selected.children // { legacy = unsupported "direct legacy package discovery is not exported by this foundation"; };
      }
    else if nested ? coercedType && nested ? finalType then
...
assert builtins.isList requests && builtins.all (request:
  builtins.isAttrs request && builtins.isList (request.path or null)
  && builtins.all builtins.isString request.path && builtins.isAttrs (request.value or null)
) requests;
